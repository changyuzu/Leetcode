...
Otherwise, the operation carries over to the next index, so we set the current index as 0
If the current index is 0, we've arrived at the first digit of the number, so we must insert 1 to the beginning
